...
class AttributeCounter():
    ...
    def get_attr_list(self):
        for obj_intensity in self.objs:
            ...
            # current_attrs['2ndmoments'] = second_moments
            rounedness = self.get_roundedness(second_moments, orientation)
            current_attrs['roundedness'] = rounedness
            # current_attrs['intensity'] = obj_intensity

            self.attr_list.append(current_attrs)
    ...
    def get_position(self, current_obj: np.ndarray):
        ...
        # NOTE: invert y axis labels
        y_axis = np.linspace(y_size, 0, y_size, endpoint=False) - 1
    ...
    def _get_centered_2nd_moments(self, current_obj: np.ndarray, position):
        ...
        y_centered = np.linspace(y_size, 0, y_size, endpoint=False) - 1 - y
...
class ImageDivider():
    ...
    def _get_label(self, x, y, pixel):
        if x == 0 or y == 0:
            # homework-specific hack
            # assumes no object lies on the edge of the image
            # so we can safely assign BG to edges
        ...
        # (x, y) is foreground
            ...
            # ABC = 1: Check if BC have the same labels
            ...
            raise ValueError(f'Unexpcted input {pixel}')
...
def label(binary_image):
    divider = ImageDivider(binary_image)
    labeled_image = divider.object_segmentation()
    return labeled_image


def get_attribute(labeled_image):
    attr_counter = AttributeCounter(labeled_image)
    attribute_list = attr_counter.get_attr_list()
    return attribute_list


def annotate_attributes(labeled_image, attribute_list):
    ARROW_LENGTH = 100
    annotated_image = np.array(labeled_image)

    def invert_y(y):
        return labeled_image.shape[0] - y

    for attr_dict in attribute_list:
        x = attr_dict['position']['x']
        y = attr_dict['position']['y']
        orientation = attr_dict['orientation']
        print(f'Object at {x:.2f}, {y:.2f}. Attrs: {attr_dict}')
...
def main(argv):
    ...
    annotated_image = annotate_attributes(labeled_image, attribute_list)
    ...
    cv2.imwrite('output/' + img_name + "_annotated.png", annotated_image)
