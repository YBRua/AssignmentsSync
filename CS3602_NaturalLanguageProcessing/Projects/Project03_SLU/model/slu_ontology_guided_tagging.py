...
class OntologyGuidedTagging(nn.Module):

    def __init__(self, config, ontology: Ontology):
        super(OntologyGuidedTagging, self).__init__()
        self.config = config
        ...
        for i in range(batch_size):
            ...
            for idx, tid in enumerate(pred):
                ...
                if (tag == 'O' or tag.startswith('B')) and len(tag_buff) > 0:
                    ...
                    value = self.get_candidate_semantics(slot, value)
                    idx_buff, tag_buff = [], []
                    if value != UNK:
